...
$action = trim($body['action'] ?? ''); // deactivate | activate | reset-devices | delete
...
switch ($action) {
    ...
    case 'delete':
        $db->prepare('DELETE FROM device_activations WHERE key_code = ?')->execute([$key]);
        $db->prepare('DELETE FROM license_keys WHERE key_code = ?')->execute([$key]);
        jsonOut(['success' => true, 'message' => "Key $key permanently deleted"]);
